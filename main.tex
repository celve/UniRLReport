...
PickScore, the optimized objective, rises monotonically at all six checkpoints on all three seeds, and the two frozen judges \emph{decelerate rather than flatten}: their first two increments carry nearly all of the gain, and both rise again at the final checkpoint on every seed, HPSv3's final increment exceeding PickScore's entire 150-to-300 movement. A decline appears on exactly one seed at one checkpoint (seed 11's HPSv3 at 250) and recovers by 300. \textbf{So the shape is deceleration, not divergence, and this row does not exhibit reward overfitting} as \texttt{GPU\_HANDOFF\_README.md} \S8.5 defines it --- \emph{``PickScore rises while independent metrics/diversity fall materially''} --- because all three independent judges rise on every seed. What falls is diversity alone, which \S8.5's taxonomy has no branch for.
\par\vspace{1mm}
\paperTODO{Still open: the E2-R VeRL-Omni reference column --- three seeds are trained and their exports audited (15/15 consecutive pairs, native $=$ export bit-identical), but no held-out evaluation was run because the estimator mismatch between the stacks is unresolved. Also outstanding: exact checkpoint revisions and released run IDs. The E1 row is a single-seed negative result: seed 11 ran its full 800-rollout budget and its final checkpoint scores below the untrained baseline, while seeds 22 and 33 were not run, so the row carries no replication.}
\end{table}
...
\emph{The training reward falls over the same span rather than rising}: the archived per-step sink gives 25-rollout window means peaking at $0.4270$ over rollouts 126--150 and declining monotonically to $0.3413$ over 251--300, across the very interval on which held-out accuracy drops.
\textbf{So this is not reward overfitting}: the optimized objective does not keep improving while the external metric falls --- the two decline together, and the turnover on both is near rollout 140.
At one point and two checkpoints the evidence carries a direction rather than a located turning point, so the supported claim is that held-out quality turns over somewhere between rollouts 150 and 300.
\textbf{No configuration measured in this work exhibits reward overfitting, and the three fail to in three different ways.} Here the training reward turns over alongside held-out accuracy. On the synchronous autoregressive run the reward is a plateau that never improves and then declines. And on the diffusion side (Section~\ref{sec:rq1}) the optimized objective rises while the independent judges also rise, decelerating and then re-accelerating rather than flattening. The signature the protocol defines --- the optimized metric improving while independent metrics fall --- is absent from all three, which is a reportable negative rather than an omission.
