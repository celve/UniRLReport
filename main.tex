...
The diffusion side behaves differently, and the contrast is the finding rather than a caveat: there the optimized objective (PickScore) rises monotonically across all six checkpoints while both frozen judges flatten (Section~\ref{sec:rq1}), which is reward overfitting; on the autoregressive path the objective itself turns over.
